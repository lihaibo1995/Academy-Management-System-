<template>
    <div>
        <div style="margin: 10px 0">
            ...
            <el-button type="success" @click="lookStuCourse(user.id)" v-if="user.role==='ROLE_STUDENT'">查看已选课表<i class="el-icon-document"></i></el-button>
            <el-button type="success" @click="lookTeacherCourses(user.id)" v-if="user.role==='ROLE_TEACHER'">查看教授课表<i class="el-icon-document"></i></el-button>
        ...
        <div style="margin: 10px 0">
            ...
            >
                <template slot-scope="scope">
                    <el-button type="danger" slot="reference"v-if="scope.row.role === 'ROLE_ADMIN'">批量删除 <i class="el-icon-remove-outline"></i></el-button>
            ...
            <el-table-column label="启用">
                <template slot-scope="scope">
                    <el-switch v-model="scope.row.state" active-color="#13ce66" inactive-color="#ccc"
                               @change="changeEnable(scope.row)" v-if=" user.role === 'ROLE_TEACHER'|| user.role === 'ROLE_ADMIN' "></el-switch>
                </template>
            </el-table-column>
            <el-table-column label="操作" width="280" align="center">
                <template slot-scope="scope">
                    <el-button type="primary" @click="selectCourse(scope.row.id)" v-if="user.role === 'ROLE_STUDENT'">选课</el-button>
                    <el-button type="danger" @click="deleteStudentCourse(scope.row.id)" v-if="user.role === 'ROLE_STUDENT'">退课</el-button>
        ...
        <el-dialog title="已选课程信息" :visible.sync="stuVis" width="30%" >
        ...
        <el-dialog title="老师教授课程" :visible.sync="teaVis" width="30%" >
...
<script>
    ...
    export default {
        ...
        methods: {
            lookStuCourse(studentId) {
                this.request.get("/course/findMyCourses/"+studentId).then(res => {
            ...
            lookTeacherCourses(teacherId) {
                this.request.get("/course/findTeacherCourses/"+teacherId).then(res => {
            ...
            selectCourse(courseId) {
                this.request.get("/course/findState/"+courseId).then(res => {
                    if (res.data.state==1) {
                        ...
                        this.$message.error("对不起，该课程暂时未开放选课！")
            ...
            deleteStudentCourse(courseId) {
                this.request.get("/course/findStudentCourse/"+courseId+ "/" + this.user.id).then(res => {
                    if (res.data==1) {
                        this.request.post('/course/deleteStudentCourse/' + courseId + "/" + this.user.id).then(res => {
                            if (res.code === '200') {
                        ...
                        this.$message.error("你还没有选这门课，请先选课！")
